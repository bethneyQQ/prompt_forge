...
2. Set environment variables in `.env` (at project root or in `prompt_forge/`):
```
...
Update history is stored in `prompt_forge/docs/update_log.json`:
...
```bash
mkdir -p prompt_forge/docs/new_provider
```
